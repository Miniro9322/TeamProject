...
// 버튼을 "눌린 모양(+색)"으로 유지하는 컴포넌트들의 공통 골격 - HeldLinkBase의 UI 전용(Animator 없이
// ButtonPressScale로 눌린 모양을 표현하는 새 버튼들) 버전. 기존 HeldLinkBase는 Animator 기반 오브젝트가
// 많이 쓰고 있어 그대로 두고, 이쪽은 별도로 분리했다.
// 무엇을 근거로 지금 눌린 상태로 볼지는 파생 클래스가 CheckHeld()로만 결정한다.
public abstract class UIHeldBase : MonoBehaviour
{
