...
// Resource/Building(생산 시설·집)은 기반시설 UI로 옮겨가 더 이상 맵에 배치되지 않는다 - 여기 남은 건 Hero뿐이다.
// resolver는 [Inject]로 늦게 들어오므로, MapGame이 Construct(주입 완료) 시점에 이 담당을 만든다.
public class UnitPlacer
{
    ...
    private GameObject Create(Placeable slot)
    {
        ...
        return hero != null ? resolver.Instantiate(slot.prefab) : null;
    }
    ...
    private void RegisterUnit(GameObject unit, PlacementArea area, Placeable slot)
    {
        ...
        area.Board.SetRangeCover(unit, area.Origin, Mathf.Max(0, slot.attackRange));
    }
