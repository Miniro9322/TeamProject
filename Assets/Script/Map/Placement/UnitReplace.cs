...
public class UnitReplace
{
    ...
    public UnitReplace(MapBoard board, UnitList unitList)
    {
        _board = board;
        _unitList = unitList;
    }
    ...
    {
        if (!_board.CanPlace(tile.Coord, _heldKind, out string reason))
        {
            message = $"{tile.Coord} {reason}";
            return false;
        }

        _board.TryPlace(tile.Coord, _heldUnit, _heldKind, yOffset, out _);
        RegisterCover(tile);
    ...
    // 재배치한 유닛의 사거리 커버를 보드에 다시 등록(건물은 사거리 없음 → 제외).
    private void RegisterCover(Tile tile)
    {
        if (_heldKind == OccupantKind.Building) return;   // 건물은 사거리 없음(널 방어 아님·실제 분기).
        _board.SetRangeCover(_heldUnit, tile.Coord, Mathf.Max(0, _heldRange));
    }
