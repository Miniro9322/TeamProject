...
// 체인 라이트닝: start에게 즉시 피해 적용 후, 이전 타겟 위치 기준 chainRange 내의
// 아직 맞지 않은 적 중 가장 가까운 하나로 전이. chainFalloff를 매 점프마다 곱해 감쇄시키며
// chainCount(최초 타격 포함)에 도달하거나 후보가 없으면 멈춘다.
public static class ChainResolver
{
    ...
    private static void ApplyDamage(GameObject go, float damage)
    {
        if (go != null && go.GetComponentInParent<IDamageAble>() is IDamageAble d)
