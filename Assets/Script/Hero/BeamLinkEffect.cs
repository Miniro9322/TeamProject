...
// Hovl_Laser.cs 참고 — 다만 Physics.Raycast로 스스로 끝점을 찾는 대신, 호출자(ContinuousBeamStrategy의
// 매 tick / Hero.SpawnChainArc의 1회 세팅)가 이미 알고 있는 두 지점을 그대로 받는다. 캐스터→타겟
// 빔과 체인 세그먼트(hits[i]→hits[i+1]) 양쪽에 동일하게 재사용된다.
